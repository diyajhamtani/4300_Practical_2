...
def use_redis(embedding_model, llm_model):
    ...
    for query in queries:
        try:
            ...
            all_rows.append(["redis", embedding_model, llm_model, query, elapsed_time, response])
        except Exception as e:
            print(f"Error processing query '{query}' in Redis: {e}")
            all_rows.append(["redis", embedding_model, llm_model, query, None, "ERROR"])
...
def use_chroma(embedding_model, llm_model):
    ...
    for query in queries:
        try:
            ...
            print(f"Elapsed time: {elapsed_time:.4f} seconds")
            all_rows.append(["chroma", embedding_model, llm_model, query, elapsed_time, response])
        except Exception as e:
            print(f"Error processing query '{query}' in Chroma: {e}")
            all_rows.append(["chroma", embedding_model, llm_model, query, None, "ERROR"])
...
def use_milvus(embedding_model, llm_model):
    ...
    for query in queries:
        try:
            ...
            print(f"Elapsed time: {elapsed_time:.4f} seconds")
            all_rows.append(["milvus", embedding_model, llm_model, query, elapsed_time, response])
        except Exception as e:
            print(f"Error processing query '{query}' in Milvus: {e}")
            all_rows.append(["milvus", embedding_model, llm_model, query, None, "ERROR"])
